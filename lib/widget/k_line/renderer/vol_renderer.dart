...
class VolRenderer extends BaseChartRenderer<VolumeEntity> {
  ...
  VolRenderer(Rect mainRect, double maxValue, double minValue, double topPadding, int fixedLength)
      : super(
            chartRect: mainRect,
            maxValue: maxValue,
            minValue: minValue,
  ...
  void drawText(Canvas canvas, VolumeEntity data, double x) {
    TextSpan span = TextSpan(
      children: [
        ...
        NumberUtil.checkNotNullOrZero(data.MA5Volume)
            ? TextSpan(text: "MA5:${NumberUtil.format(data.MA5Volume)}    ", style: getTextStyle(ChartColors.ma5Color))
            : Container(),
        NumberUtil.checkNotNullOrZero(data.MA10Volume)
            ? TextSpan(
                text: "MA10:${NumberUtil.format(data.MA10Volume)}    ", style: getTextStyle(ChartColors.ma10Color))
            : Container(),
      ],
    );
